...
public class BossGearWhealNote : BossNote
{
    [System.Serializable]
    ...
    protected override void UpdateAnimation()
    {
        ...
        float animationNormalizedHit = 0.0f;
        switch (data.position)
        {
            case NotePosition.Air:
            animationNormalizedHit = airHitRatio;
            break;

            case NotePosition.Road:
            animationNormalizedHit = roadHitRatio;
            break;
        }

        // 1. invRatio가 0일때 정규화된 애니메이션 시간은 0입니다.
        // 2. invRatio가 1일때 정규화된 애니메이션 시간은 animationNormalizedHit일때 입니다.
        // ax + b = y : a * invRatio + b = animationRatio 라고 할 때
        // 1. a * 0 + b = 0
        // 2. a * 1 + b = animationNormalizedHit
        // a = animationNormalizedHit
        // b = 0
        // animationNormalizedHit * invRatio = animationRatio

        float speedMultiplier = GetSpeedMultiplier();
        float invRatio = 1.0f - ratio * speedMultiplier;
        float animationRatio = animationNormalizedHit * invRatio;
    ...
    protected override void UpdateBossAnimationData()
    {
        ...
        // invRatio = 1.0f - ratio * speedMultiplier
